...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_stage'])) {
    ...
    if ($isSubmittedVal == 1) { // Only validate on "Submit" action
        ...
        if (empty($approvedAt) || empty($remark)) {
...
<head>
    ...
    <style>
        /* Basic styling for readonly fields to look distinct */
        .readonly-office-field {
            padding: 4px; /* Decreased padding from 8px to 4px */
        ...
        /* Style for the table cells to align content */
        table td {
            ...
            form.addEventListener('submit', function(event) {
                ...
                if (buttonText === 'Submit' && !submitButton.disabled) {
                    ...
                    if (!approvedAtField || !approvedAtField.value) {
                        isValid = false;
                        errorMessage = "Approved Date/Time is required.";
                    }
                    ...
                    if (isAdmin && !isPurchaseRequest && !currentCreatedValue) {
                        isValid = false;
                        errorMessage = "Created Date/Time is required for this stage.";
                    }

                    if (!isValid) {
                        event.preventDefault();
                        alert(errorMessage); // Using alert as per previous instructions
                    }
                }
            });
        });
                ...
                <tbody>
                    ...
                    foreach ($stagesOrder as $index => $stage):
                        ...
                        if ($value_office_id_for_stage !== null && isset($officeList[$value_office_id_for_stage])) {
                            $displayOfficeName = htmlspecialchars($officeList[$value_office_id_for_stage]);
                        }
